...
public class SetupDataQualityIndicatorBySiteReport {
	...
	private void createIndicators(
		...
		SqlCohortDefinition patientsInPMTCTTooLong = new SqlCohortDefinition(
				"select distinct patient_id from patient_program pp,program p where pp.program_id=p.program_id and p.name='"
						+ pmtct.getName()
						+ "' and pp.date_enrolled<'"
						+ GetDate.getCalendarMonthDate(-8)
		...
		SqlCohortDefinition patientsInPMTCTCCMTooLong = new SqlCohortDefinition(
				"select distinct patient_id from patient_program pp,program p where pp.program_id=p.program_id and p.name='"
						...
						+ "' and pp.date_enrolled<'"
						+ GetDate.getCalendarMonthDate(-19)
						+ "' and pp.voided=false and pp.date_completed is null");
		...
		for (Concept concept : artDrugsconcepts) {
			stringOfIdsOfConcepts = stringOfIdsOfConcepts + ","
					+ concept.getId();
			/*
			 * List<Drug>
			 * drugs=Context.getConceptService().getDrugsByConcept(concept);
			 * for(Drug drug:drugs){ artDrugs.add(drug);
			 * log.debug("Drug: "+drug.getName()); }
			 */
		}
		...
		log.debug("##############################: Size of Array Of TB Drug concepts:"
				+ tbDrugsconcepts.size());
		for (Concept concept : tbDrugsconcepts) {
			stringOfIdsOfTbDrugsConcepts = stringOfIdsOfTbDrugsConcepts + ","
					+ concept.getId();
			log.debug("Drug concept id: " + concept.getId());
		}
		...
		SqlCohortDefinition patientsWithIMBOrPCIdentifer = new SqlCohortDefinition(
				"select distinct patient_id from patient_identifier where identifier_type in ("
						+ imbPCIdentifiersID + ") and voided=0");
		...
		patientsWithoutIMBOrPCIdentiferWithAnyEncounterLastYearFromNow
				.getSearches().put("2",
						new Mapped(patientsWithIMBOrPCIdentifer, null));
		patientsWithoutIMBOrPCIdentiferWithAnyEncounterLastYearFromNow
		...
		// ======================================================================================
		// 14. Patients with a visit in last 12 months who do not have a
		// correctly structured address
		// ======================================================================================

		SqlCohortDefinition patientsWithNoStructuredAddress = new SqlCohortDefinition(
				"select distinct(p.patient_id) from patient p,person_address pa where p.patient_id=pa.person_id and p.voided=0 and (pa.state_province is null or pa.county_district is null or pa.city_village is null or pa.neighborhood_cell is null or pa.address1 is null)");
		...
		ProgramEnrollmentCohortDefinition oractaWithNoWorkFLow = getProgramEnrollmentbyGP(
				"patientInHivProgram", getGProp("dq.report.oractaprogram"));
		oractaWithNoWorkFLow.addParameter(new Parameter("enrolledOnOrBefore",
		...
		patientExitedfromcareinPrograms.getSearches().put(
				"2",
				new Mapped(oractaWithNoWorkFLow, ParameterizableUtil
						.createParameterMappings("enrolledOnOrBefore=${now}")));
		patientExitedfromcareinPrograms.getSearches().put(
				"3",
				new Mapped(patientDied, ParameterizableUtil
		...
		// ======================================================================================
		// 16. Patients who status is transferred out but is currently enrolled
		// in program
		// ======================================================================================
		...
		patientWithLessThan12
				.setQuery("select w.person_id from (select * from (select o.person_id,o.value_numeric from obs o,concept c where o.concept_id= c.concept_id and c.uuid='"
						+ Context.getAdministrationService().getGlobalProperty(
								"rwandareports.HEIGHTConceptuuid")
						+ "' order by o.obs_datetime desc) as lastheight group by lastheight.person_id) h,(select * from (select o.person_id,o.value_numeric from obs o,concept c where o.concept_id= c.concept_id and c.uuid='"
						+ Context.getAdministrationService().getGlobalProperty(
								"rwandareports.WEIGHTConceptuuid")
						+ "' order by o.obs_datetime desc) as lastweight group by lastweight.person_id) w where w.person_id=h.person_id and ROUND(((w.value_numeric*10000)/(h.value_numeric*h.value_numeric)),2)<=12.0");

		SqlCohortDefinition patientWithMoreThan35 = new SqlCohortDefinition();
		patientWithMoreThan35.setName("patientWithMoreThan35");
		patientWithMoreThan35
				.setQuery("select w.person_id from (select * from (select o.person_id,o.value_numeric from obs o,concept c where o.concept_id= c.concept_id and c.uuid='"
						+ Context.getAdministrationService().getGlobalProperty(
								"rwandareports.HEIGHTConceptuuid")
						+ "' order by o.obs_datetime desc) as lastheight group by lastheight.person_id) h,(select * from (select o.person_id,o.value_numeric from obs o,concept c where o.concept_id= c.concept_id and c.uuid='"
						+ Context.getAdministrationService().getGlobalProperty(
								"rwandareports.WEIGHTConceptuuid")
						+ "' order by o.obs_datetime desc) as lastweight group by lastweight.person_id) w where w.person_id=h.person_id and ROUND(((w.value_numeric*10000)/(h.value_numeric*h.value_numeric)),2)>=35.0");

		CompositionCohortDefinition patientsWithBMIMoreThan35LessThan12 = new CompositionCohortDefinition();
		patientsWithBMIMoreThan35LessThan12
				.setName("DQ: patients with BMI less than 12 or more that 35");
		patientsWithBMIMoreThan35LessThan12.getSearches().put("1",
				new Mapped(patientWithLessThan12, null));
		patientsWithBMIMoreThan35LessThan12.getSearches().put("2",
				new Mapped(patientWithMoreThan35, null));
		;
		patientsWithBMIMoreThan35LessThan12.setCompositionString("1 OR 2");

		CohortIndicator patientsWithBMIMoreThan35LessThan12Indicator = Indicators
				.newCountIndicator("Number with BMI value",
						patientsWithBMIMoreThan35LessThan12, null);
		...
		patientsOnArtbeforeHivEnrollment
				.setName("patientsOnArtbeforeHivEnrollment");
		patientsOnArtbeforeHivEnrollment
				.setQuery("SELECT pp.patient_id "
		...
		patientswithouttransferInForm
				.setQuery(" SELECT en.patient_id FROM encounter en, form f WHERE f.form_id=en.form_id AND en.encounter_type=f.encounter_type AND f.form_id=132 AND en.encounter_type=28 ");
		...
		patientsWithinvaliddatesandmissingforms.getSearches().put("1",
				new Mapped(patientsOnArtbeforeHivEnrollment, null));
		patientsWithinvaliddatesandmissingforms.getSearches().put("2",
		...
		reportDefinition
				.addIndicator(
						"1",
						"patients who are in Pediatric and Adult HIV program AND on ART whose accompagnateur is not listed in EMR",
						patientsInHIVOnARTWithoutAccompIndicator);
		reportDefinition.addIndicator("2",
				"Patients enrolled in PMTCT Pregnancy for more than 8 months",
		...
		reportDefinition.addIndicator("13", "Patients over 100 years old",
		...
		reportDefinition
				.addIndicator(
						"15",
						"Patients whose status status deceased but enrolled in program",
		...
		reportDefinition.addIndicator("18", "Patients with a BMI <12  or  >35",
				patientsWithBMIMoreThan35LessThan12Indicator);
		reportDefinition
				.addIndicator(
						"19",
	...
	private void setUpProperties() {
		...
		heartFailure = gp
				.getProgram(GlobalPropertiesManagement.HEART_FAILURE_PROGRAM);
		...
		hypertention = gp
				.getProgram(GlobalPropertiesManagement.HYPERTENTION_PROGRAM);
		...
		tbDrugsconcepts = gp
				.getConceptsByConceptSet(GlobalPropertiesManagement.TB_TREATMENT_DRUGS);
	...
	public ProgramEnrollmentCohortDefinition getProgramEnrollmentbyGP(
			String cohortName, int programId) {
		...
		Program program = Context.getProgramWorkflowService().getProgram(
				programId);
		if (program == null)
			throw new APIException("ProgramId " + programId + " does not exist");
